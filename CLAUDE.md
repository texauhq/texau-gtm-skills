...
2. **Every skill starts with the preflight preamble.** Skips equal fragility. Copy the pattern from `skills/richapi-gtm/SKILL.md`.
...
./bin/richapi-skills-sync          # refresh _lib/mcp-catalog.json from live MCP
./bin/richapi-skills-sync --check  # exit 1 if catalog >7 days old
./bin/richapi-skills-preflight     # emit KEY: value health report (used by skills)
./bin/richapi-skills-config get|set|unset|list <key> [value]
...
5. Update the router: add a row to the routing table in `skills/richapi-gtm/SKILL.md`.
...
2. Run `./bin/richapi-skills-sync` locally — it rebuilds `_lib/mcp-catalog.json`.
...
- The `bin/richapi-skills-preflight` script checks GitHub for a newer top-level `VERSION` and surfaces `UPGRADE: <ver>` through the preamble. Users see an unobtrusive offer at session end.
...
├── bin/
│   ├── richapi-skills-sync           # refresh catalog from live MCP
│   ├── richapi-skills-preflight      # health check → KEY: value pairs
│   └── richapi-skills-config         # user prefs (proactive, sync_on_start, etc.)
├── _lib/
...
├── skills/
│   ├── richapi-gtm/SKILL.md          # entry router
