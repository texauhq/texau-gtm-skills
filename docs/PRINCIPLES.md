...
Only the entry `richapi-gtm` skill reads the full `_lib/mcp-catalog.json` into context. Sub-skills read only the slices they need (typically `tools[]` filtered by category). This keeps sub-skill context windows lean.

## 10. No hidden state, no hidden writes

The pack stores exactly three things on disk, all under `$richapi_SKILLS_HOME` (default `~/.richapi-skills/`):
